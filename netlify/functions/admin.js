...
async function showConv(user,quiet){CUR=user;drawUsers();var u=users().find(x=>x.userId===user)||{userId:user};$('#chat').classList.add('open');history.replaceState(null,'','#u='+user);
 ...
 p.innerHTML='<div class="ph"><button class="back" data-back="1" aria-label="חזרה">→</button><div class="av">'+esc(initials(d.name?{name:d.name}:u))+'</div><div class="mid"><div class="nm">'+esc(d.name||'ללא שם')+'</div><div class="sub">'+esc(phone(user))+' · '+(u.received||0)+' התקבלו · '+(u.sent||0)+' נשלחו · עלות '+usd(u.estimatedUsd)+'</div></div><button data-step="-1" title="השיחה הקודמת" aria-label="השיחה הקודמת"'+(i<=0?' disabled':'')+'>↑</button><button data-step="1" title="השיחה הבאה" aria-label="השיחה הבאה"'+(i>=users().length-1?' disabled':'')+'>↓</button></div><div class="msgs" id="msgs">'+(html||'<p class="hint">אין הודעות</p>')+'</div>';
