// One-shot demo job (restore). Triggered by deploy-succeeded only when the deployed commit title contains [demo-restore].
import { getStore } from "@netlify/blobs";
import { netlifyStore } from "../../src/store.js";
import { demoRestore } from "../../src/demo.js";
...
export default async req=>{if((req.headers.get("authorization")||"")!==`Bearer ${internalToken()}`){console.log(JSON.stringify({event:"demo_restore",status:"unauthorized"}));return new Response("unauthorized",{status:401});}
 ...
 const r=await demoRestore(store,{deliverFn});console.log(JSON.stringify({event:"demo_restore",...r}));return new Response("ok");};
