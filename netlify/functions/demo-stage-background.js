// One-shot demo job (stage). Triggered by deploy-succeeded only when the deployed commit title contains [demo-stage].
import { getStore } from "@netlify/blobs";
import { netlifyStore } from "../../src/store.js";
import { demoStage } from "../../src/demo.js";
...
export default async req=>{if((req.headers.get("authorization")||"")!==`Bearer ${internalToken()}`){console.log(JSON.stringify({event:"demo_stage",status:"unauthorized"}));return new Response("unauthorized",{status:401});}
 ...
 const r=await demoStage(store,{deliverFn});console.log(JSON.stringify({event:"demo_stage",...r}));return new Response("ok");};
