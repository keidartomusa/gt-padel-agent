...
export default async req=>{if((req.headers.get("authorization")||"")!==`Bearer ${internalToken()}`)return new Response("unauthorized",{status:401});
 const r=await diagMessages(netlifyStore(getStore({name:"gt-padel-matching",consistency:"strong"})),{from:"2026-09-24T07:40:00Z",to:"2026-09-24T08:05:00Z"});console.log(JSON.stringify({event:"diag_messages",users:r.users.length}));return new Response("ok");};
