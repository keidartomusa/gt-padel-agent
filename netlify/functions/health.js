...
export default async () => {let lastReset=null,lastSeed=null,lastDemo=null,lastDiag=null,lastCourtTemplate=null,lastWabaBillingDiagnostic=null,lastCourtScan=null,lastCourtReleases=[];try{const st=getStore({name:"gt-padel-matching",consistency:"strong"});lastReset=await st.get("meta/last-reset",{type:"json"});lastSeed=await st.get("meta/last-seed",{type:"json"});lastDemo=await st.get("meta/last-demo",{type:"json"});lastDiag=await st.get("meta/last-diag",{type:"json"});lastCourtTemplate=await st.get("meta/court-template",{type:"json"});lastWabaBillingDiagnostic=await st.get("meta/waba-billing-diagnostic",{type:"json"});
 ...
 return new Response(JSON.stringify({ok:true,service:"gt-padel-agent",build:BUILD,lastReset,lastSeed,lastDemo,lastDiag,lastCourtTemplate,lastWabaBillingDiagnostic,lastCourtScan,lastCourtReleases}),{headers:{"content-type":"application/json"}});};
