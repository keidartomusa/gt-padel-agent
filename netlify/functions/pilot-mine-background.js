...
export default async req=>{const auth=req.headers.get("authorization")||"";
 if(auth!==`Bearer ${internalToken()}`&&!(process.env.ADMIN_DASHBOARD_TOKEN&&auth===`Bearer ${process.env.ADMIN_DASHBOARD_TOKEN}`)){console.log(JSON.stringify({event:"pilot_mine",status:"unauthorized"}));return new Response("unauthorized",{status:401});}
