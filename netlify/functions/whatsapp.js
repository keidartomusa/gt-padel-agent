...
export default async req=>{
 ...
 for(const entry of body.entry||[])for(const change of entry.changes||[])for(const msg of change.value?.messages||[]){const input=messageInput(msg);if(!input)continue;const phoneId=change.value?.metadata?.phone_number_id;await sendReaction(msg.from,msg.id,"👍",phoneId);await sendTyping(msg.id,phoneId);const contact=change.value?.contacts?.find(x=>x.wa_id===msg.from),response=await routeIncoming({userId:msg.from,displayName:contact?.profile?.name||"שחקן/ית",...input,store});await sendResponse(msg.from,response,phoneId);for(const n of response.notifications||[])await sendResponse(n.to,n.response,change.value?.metadata?.phone_number_id);}
