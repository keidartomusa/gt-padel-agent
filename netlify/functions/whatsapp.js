...
export default async req=>{
 ...
 for(const {entry,change,venue} of routes)for(const msg of change.value?.messages||[]){const store=storeFor(venue);const input=messageInput(msg);if(!input)continue;const phoneId=change.value?.metadata?.phone_number_id;const typing=await sendTyping(msg.id,phoneId);console.log(JSON.stringify({event:"whatsapp_inbound",messageId:msg.id,from:msg.from,type:msg.type,phoneIdPresent:Boolean(phoneId)}));await logInbound(store,msg.from,msg,input);const contact=change.value?.contacts?.find(x=>x.wa_id===msg.from),response=await routeIncoming({userId:msg.from,displayName:contact?.profile?.name||"שחקן/ית",...input,store,venue});assertVenueResponse(venue,response);const sent=await sendResponse(msg.from,response,phoneId);await logOutbound(store,msg.from,response,sent);console.log(JSON.stringify({event:"whatsapp_outbound",messageId:msg.id,typing,sent}));for(const n of response.notifications||[]){assertVenueResponse(venue,n.response);const d=await deliver(store,n.to,n.response,{send:(to,r)=>sendResponse(to,r,phoneId),sendTemplate:(to,t)=>sendTemplate(to,t.name,t.language,t.payloads,phoneId)});console.log(JSON.stringify({event:"whatsapp_notification",to:n.to,mode:d.mode,sent:d.sent,reason:d.reason}));}}
