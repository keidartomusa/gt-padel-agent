...
export default async req=>{
 ...
 for(const entry of body.entry||[])for(const change of entry.changes||[])for(const msg of change.value?.messages||[]){const input=messageInput(msg);if(!input)continue;const contact=change.value?.contacts?.find(x=>x.wa_id===msg.from),result=await handleConversation({userId:msg.from,displayName:contact?.profile?.name||"שחקן/ית",...input,store});const response=result.mode==="availability"?{text:`${await answer(result.query)}\n\nכתבו *תפריט* בכל שלב כדי לראות את האפשרויות.`,buttons:[{id:"menu",title:"לתפריט"}]}:result;await sendResponse(msg.from,response,change.value?.metadata?.phone_number_id);for(const n of response.notifications||[])await sendResponse(n.to,n.response,change.value?.metadata?.phone_number_id);}
