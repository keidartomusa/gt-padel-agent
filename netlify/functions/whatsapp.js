...
import { sendReaction, sendResponse, sendTyping } from "../../src/whatsapp.js";
...
export default async req=>{
 ...
 for(const entry of body.entry||[])for(const change of entry.changes||[])for(const msg of change.value?.messages||[]){const input=messageInput(msg);if(!input)continue;const phoneId=change.value?.metadata?.phone_number_id;console.log(JSON.stringify({event:"whatsapp_inbound",messageId:msg.id,from:msg.from,type:msg.type,phoneIdPresent:Boolean(phoneId)}));const reaction=await sendReaction(msg.from,msg.id,"👍",phoneId),typing=await sendTyping(msg.id,phoneId);const contact=change.value?.contacts?.find(x=>x.wa_id===msg.from),response=await routeIncoming({userId:msg.from,displayName:contact?.profile?.name||"שחקן/ית",...input,store});const sent=await sendResponse(msg.from,response,phoneId);console.log(JSON.stringify({event:"whatsapp_outbound",messageId:msg.id,reaction,typing,sent}));for(const n of response.notifications||[])await sendResponse(n.to,n.response,phoneId);}
