...
let clicks = cta ? [{ id: "demo-click", clickedAt: new Date(at0 - 17 * 60000).toISOString(), source: cta.get("source"), date: cta.get("date"), time: cta.get("time"), duration: Number(cta.get("duration")), price: Number(cta.get("price")), status: "clicked", userId: cta.get("user"), userName: names[cta.get("user")] || null }] : [];
const conv = {};
if (process.env.RICH === "1") (await import("./rich.mjs")).enrich({ requests, connections, profiles, names, clicks, conv, at0 });
...
const theme = process.env.THEME ? `<style>${fs.readFileSync(process.env.THEME, "utf8")}</style>` : "";
fs.writeFileSync(out, html.replace("<script>", mock + "<script>").replace("</head>", "").replace("<header", theme + "<header") + tabJs);
