...
const KEEP = { 1: [2], 2: [1, 2, 4], 3: [4, 8, 12, 14], 4: [2, 4], 5: [1, 3], 6: [1], 7: [1], 8: [4], 9: [10, 12] };
...
shot(`<div class="card"><h1>GT PADEL בוואטסאפ</h1><p>מגרש פנוי ושותפים למשחק - בהודעה אחת</p><p style="font-size:16px;opacity:.75;margin-top:30px">זמינות ומחירים אמיתיים מהמועדון · משתמשי דמו בדויים</p></div>`, 2.2);
for (const sc of ev.scenes) {
  const label = `${sc.id} / ${ev.scenes.length}`;
  // Message by message, only the key beats (KEEP: message counts shown per scene; max 1:30 per Tom). Typing bubble before bot replies; each step has an action + meaning line (steps.json).
  for (const k of KEEP[sc.id] || []) { const msgs = sc.chat.slice(0, k), last = msgs[k - 1], step = STEPS[sc.id]?.[k - 1];
    if (!last.me) shot(cap(label, sc.caption, step) + phone(sc.phone, `<div class="chat">${msgs.slice(0, -1).map(bub).join("")}<div class="typ">•••</div></div>`), 0.4);
    shot(cap(label, sc.caption, step) + phone(sc.phone, `<div class="chat">${msgs.map(bub).join("")}</div>`), last.me ? 1.1 : Math.min(3.4, 1.8 + String(last.r?.text || "").length / 160)); }
  if (sc.id === 2 && existsSync("/tmp/demo/mp-book.png")) shot(cap(label, "ההזמנה עצמה - באתר המועדון", "הכפתור פותח את מסך ההזמנה האמיתי ב-Matchpointer, עם המגרש והשעה שנבחרו") + `<div class="phone"><div class="scr"><div class="img" style="background-image:url(file:///tmp/demo/mp-book.png)"></div></div></div>`, 3.2);
}
for (const d of ["overview", "live", "chats", "clicks"]) { const p = `/tmp/demo/dash-${d}.png`; if (existsSync(p)) shot(cap("דשבורד", { overview: "דשבורד למועדון - תמונת מצב", live: "בקשות וחיבורים", chats: "כל השיחות במקום אחד", clicks: "לחיצות על 'להזמנה'" }[d], { overview: "משתמשים, בקשות, משחקים ולחיצות - במבט אחד", live: "כל משחק פתוח ומי כבר בפנים", chats: "כל שיחה עם הבוט, כמו בוואטסאפ", clicks: "מי לחץ על מה - הלחיצות לא נספרות כהזמנות" }[d]) + `<div style="position:absolute;top:215px;left:10px;right:10px;height:722px;border-radius:14px;overflow:hidden;box-shadow:0 12px 40px #0006;background:#fff url(file://${p}) no-repeat center top/100% auto"></div>`, 3); }
shot(`<div class="card"><h1>GT PADEL</h1><p>שואלים על מגרש, מוצאים שותפים, ומזמינים באתר המועדון</p></div>`, 2.2);
