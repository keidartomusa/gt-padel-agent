...
const phone = (who, inner) => `<div class="phone"><div class="scr"><div class="hd"><div class="av">GT</div><div><div class="nm">GT PADEL</div><div class="sb">הטלפון של ${NAMES[who]} (משתמשת דמו)</div></div></div>${inner}</div></div>`;
...
for (const d of ["overview", "requests", "conversations", "clicks"]) { const p = `/tmp/demo/dash-${d}.png`; if (existsSync(p)) shot(cap("דשבורד", { overview: "דשבורד למועדון - תמונת מצב", requests: "בקשות וחיבורים", conversations: "כל השיחות במקום אחד", clicks: "לחיצות על 'להזמנה' (לא נספרות כהזמנות)" }[d]) + `<div class="phone" style="width:500px"><div class="scr"><div class="img" style="background-image:url(file://${p})"></div></div></div>`, 4); }
