...
async function scenario(title, group, actors, steps, expect = {}) {
  ...
  for (const step of steps) {
    let [k, s] = step; const u = users[k];
    if (s.ifAsked && !s.ifAsked.test(last[k]?.text || "")) continue;
    if (s.name) { const lt = last[k]?.text || ""; if (/באיזה שם/.test(lt)) s = { text: u.displayName }; else if (/בתור/.test(lt)) s = { tap: /^כן$/ }; else continue; }
    let st = s.text === "@name" ? { text: u.displayName } : s;
    if (s.tap) { const opts = [...(last[k]?.buttons || []), ...(last[k]?.rows || [])]; const o = opts.find(x => s.tap.test(x.title.replace(/[\u2066-\u2069]/g, ""))); if (!o && s.opt) continue; if (!o) { problems.push(`${title}: ${k} can't tap ${s.tap} (options: ${opts.map(x => x.title).join(" | ")})`); break; } st = { actionId: o.id, title: o.title }; }
...
const S = [];
const pc = (party, court) => new RegExp(party.source + ".*" + (court.source.includes("כן") ? "יש מגרש" : "בלי מגרש"));
const NM = k => [k, { name: true }];
const A = (id, n) => [id, n];
// ---------- 1. entry & identity ----------
...
S.push(await scenario("2. שם: שאלה חד-פעמית ושינוי שם עם אישור", "כניסה", { a: A("972501110002", "Tom K") }, [
  ["a", { text: "שלום" }], ["a", { tap: /מציאת שחקנים/ }], ["a", { tap: /חסרים לי שחקנים/ }], ["a", { tap: /^3/, opt: true }], ["a", { tap: pc(/רק אני/, /^כן$/) }], ["a", { text: "מחר אחרי 19:00" }], ["a", { tap: /90/ }], ["a", { text: "תום" }],
...
S.push(await scenario("5. חסרים לי שחקנים: בלי מגרש (שאלת גמישות) ועם מגרש (בלי גמישות)", "מציאת שחקנים", { a: A("972501110005", "איתי"), b: A("972501110006", "שירה") }, [
  ["a", { text: "שלום" }], ["a", { tap: /מציאת שחקנים/ }], ["a", { tap: /חסרים לי שחקנים/ }], ["a", { tap: /^3-3\.5|^3/ }], ["a", { tap: pc(/אני ועוד אחד/, /^לא$/) }], ["a", { text: "מחר אחרי 19:00" }], ["a", { tap: /90/ }], ["a", { tap: /^שעה$/ }], NM("a"),
  ["b", { text: "שלום" }], ["b", { tap: /מציאת שחקנים/ }], ["b", { tap: /חסרים לי שחקנים/ }], ["b", { tap: /^1|^2/ }], ["b", { tap: pc(/שלושה/, /^כן$/) }], ["b", { text: "שבת בבוקר" }], ["b", { tap: /60/ }], NM("b")]));
S.push(await scenario("6. משחק קבוע כל שבוע", "מציאת שחקנים", { a: A("972501110007", "עומר") }, [
  ["a", { text: "שלום" }], ["a", { tap: /מציאת שחקנים/ }], ["a", { tap: /משחק קבוע/ }], ["a", { tap: /^3/ }], ["a", { tap: pc(/רק אני/, /^כן$/) }], ["a", { text: "שני ורביעי אחרי 20:00" }], ["a", { tap: /20:00|מאיזו/, opt: true }], ["a", { tap: /גמיש|90/, opt: true }], NM("a")]));
// ---------- 4. multi-user matching & coordination ----------
let NAMES = {};
const reg = (k, lvl, when, party, court = /^כן$/) => [[k, { text: "שלום" }], [k, { tap: /מציאת שחקנים/ }], [k, { tap: /חסרים לי שחקנים/ }], [k, { tap: lvl }], [k, { tap: pc(party, court) }], [k, { text: when }], [k, { tap: /90/ }], [k, { tap: /^שעה$/, ifAsked: /גמישים/ }], NM(k)];
...
S.push(await scenario("8. לוח משחקים: הצטרפות לבקשה פתוחה ← אישור ← החלפת מספרים", "התאמות ותיאום", { a: A("972501110021", "טל"), b: A("972501110022", "גיל") }, [
  ...
  ["b", { text: "שלום" }], ["b", { tap: /מציאת שחקנים/ }], ["b", { tap: /להצטרף למשחק/ }], ["b", { tap: /מחר בערב/ }], ["b", { tap: /טל/ }], ["b", { name: true }], ["a", { tap: /כן, לחבר/ }]]));
S.push(await scenario("9. לוח ריק: פתיחת בקשה אוטומטית (פעם ראשונה, ואז עם רמה ומשך מהבקשה הקודמת), וביטול", "התאמות ותיאום", { a: A("972501110031", "ליאור"), b: A("972501110032", "מאיה") }, [
  ["a", { text: "שלום" }], ["a", { tap: /מציאת שחקנים/ }], ["a", { tap: /להצטרף למשחק/ }], ["a", { text: "מחר אחרי 18:00" }], ["a", { tap: /^3/, opt: true }], ["a", { tap: pc(/רק אני/, /^כן$/), opt: true }], ["a", { tap: /90/, opt: true }], NM("a"),
  ["a", { tap: /לתפריט/ }], ["a", { tap: /מציאת שחקנים/ }], ["a", { tap: /להצטרף למשחק/ }], ["a", { text: "שבת בבוקר" }], ["a", { tap: pc(/אני ועוד אחד/, /^כן$/) }], ["a", { tap: /90/, opt: true }], NM("a"),
...
S.push(await scenario("10. הבקשות שלי: רשימה, עריכת כל שדה, מחיקה", "ניהול בקשות", { a: A("972501110041", "אלון") }, [
  ...reg("a", /^3/, "ראשון בערב", /רק אני/, /^לא$/), ["a", { tap: /^שעה$/, opt: true }], NM("a"),
  ["a", { tap: /בקשה נוספת/ }], ["a", { tap: /חסרים לי שחקנים/ }], ["a", { tap: /^3/, opt: true }], ["a", { tap: pc(/רק אני/, /^כן$/) }], ["a", { text: "שישי בבוקר" }], ["a", { tap: /60/, opt: true }],
  ["a", { tap: /הבקשות שלי/ }], ["a", { tap: /חמישי|שישי/ }], ["a", { tap: /עריכה/ }], ["a", { tap: /רמה/ }], ["a", { tap: /^4/ }],
  ["a", { text: "הבקשות שלי" }], ["a", { tap: /חמישי|שישי/ }], ["a", { tap: /עריכה/ }], ["a", { tap: /יום ושעה/ }], ["a", { text: "שבת ב-9" }], ["a", { tap: /בבוקר/ }],
  ["a", { text: "הבקשות שלי" }], ["a", { tap: /שבת|חמישי|שישי/ }], ["a", { tap: /עריכה/ }], ["a", { tap: /משך/ }], ["a", { tap: /120/ }],
  ["a", { text: "הבקשות שלי" }], ["a", { tap: /שבת|חמישי|שישי/ }], ["a", { tap: /עריכה/ }], ["a", { tap: /מספר שחקנים/ }], ["a", { tap: /שלושה/ }],
  ["a", { text: "הבקשות שלי" }], ["a", { tap: /שבת|חמישי|שישי/ }], ["a", { tap: /עריכה/ }], ["a", { tap: /מגרש/ }], ["a", { tap: /^כן$/ }],
  ["a", { text: "הבקשות שלי" }], ["a", { tap: /שבת|חמישי|שישי/ }], ["a", { tap: /מחיקה/ }], ["a", { tap: /^לא$/ }], ["a", { tap: /שבת|חמישי|שישי/ }], ["a", { tap: /מחיקה/ }], ["a", { tap: /כן, למחוק/ }],
  ["a", { text: "תפריט" }], ["a", { text: "הבקשות שלי" }]]));
S.push(await scenario("11. מגבלת 5 בקשות", "ניהול בקשות", { a: A("972501110051", "רותם") }, [
  ...reg("a", /^3/, "מחר אחרי 19:00", /רק אני/), ...["שישי בבוקר", "שבת בבוקר", "ראשון בערב", "שני בערב"].flatMap(w => [["a", { tap: /בקשה נוספת/ }], ["a", { tap: /חסרים לי שחקנים/ }], ["a", { tap: /^3/, opt: true }], ["a", { tap: pc(/רק אני/, /^כן$/) }], ["a", { text: w }], ["a", { tap: /90/, opt: true }], NM("a")]),
...
S.push(await scenario("13. בלי מגרש, ואין מגרש פנוי בחלון: הבקשה לא מתפרסמת, ניסיון זמן אחר", "מציאת שחקנים", { a: A("972501110071", "הילה") }, [
  ...reg("a", /^3/, "מחר בערב", /רק אני/, /^לא$/), ["a", { tap: /^שעה$/, opt: true }], NM("a"), ["a", { tap: /זמן אחר/, ifAsked: /לא פורסמה/ }], ["a", { text: "ראשון בערב", ifAsked: /מתי/ }], ["a", { tap: /^שעה$/, ifAsked: /גמישים/ }]]));
// ---------- 6. settings ----------
...
S.push(await scenario("14. שלושה מתאימים לאותו אדם: כל אחד מקבל הצעה אחת, הקבוצה מתמלאת, בקשה ממתינה נסגרת", "התאמות ותיאום", { a: A("972501110081", "אורי"), b: A("972501110082", "בר"), c: A("972501110083", "גיא"), e: A("972501110085", "אלה"), d: A("972501110084", "דור") }, [
  ...reg("a", /^3-3\.5/, "מחר אחרי 19:00", /רק אני/), ...reg("b", /^3-3\.5/, "מחר אחרי 19:00", /רק אני/), ...reg("c", /^3-3\.5/, "מחר אחרי 19:00", /רק אני/),
  ...reg("d", /^3-3\.5/, "מחר אחרי 19:00", /רק אני/),
  ["a", { tap: /רוצה להתחבר/ }], ["d", { tap: /כן, לחבר/ }], ["b", { tap: /רוצה להתחבר/ }], ["d", { tap: /כן, לחבר/ }],
  ["c", { tap: /רוצה להתחבר/ }], ["e", { text: "שלום" }], ["e", { tap: /מציאת שחקנים/ }], ["e", { tap: /להצטרף למשחק/ }], ["e", { tap: /מחר בערב/ }], ["e", { tap: /דור/ }], NM("e"),
  ["d", { tap: /כן, לחבר/ }]]));
