...
export async function handleConversation({userId,displayName="שחקן/ית",text="",actionId,store,now=new Date(),availabilityFn=findAvailability}){
 ...
 if(input.startsWith("rm:")){const r=await memberReq(input.slice(3));if(!r)return btn("הבקשה כבר לא פעילה.",[MY_REQ,MENU]);const who=members(r).filter(m=>m.id!==userId&&m.id!==r.userId);if(!who.length)return btn("אין בקבוצה שחקן שאפשר להסיר.",[MY_REQ,MENU]);return list("את מי להסיר מהמשחק?",who.map(m=>({id:`rmu:${r.id}:${m.id}`,title:(m.name||"שחקן").slice(0,24)})).concat({id:`req:${r.id}`,title:"חזרה"}));}
 if(input.startsWith("rmu:")){const[,rid,uid]=input.split(":"),r=await memberReq(rid),m=r&&members(r).find(x=>x.id===uid&&x.id!==r.userId);if(!m)return btn("הבקשה כבר לא פעילה.",[MY_REQ,MENU]);return btn(`להסיר את ${m.name} מהמשחק (${whenOf(r)})?`,[{id:`rmok:${rid}:${uid}`,title:"כן, להסיר"},{id:`req:${rid}`,title:"לא"}]);}
 if(input.startsWith("rmok:")){const[,rid,uid]=input.split(":"),r=await memberReq(rid),idx=r?(r.joined||[]).indexOf(uid):-1;if(idx<0||uid===userId)return btn("הבקשה כבר לא פעילה.",[MY_REQ,MENU]);
   const name=(r.joinedNames||[])[idx]||"",conn=(await store.list("connection/")).map(x=>x.value).find(c=>c?.requestId===rid&&c.fromUserId===uid&&c.status==="accepted"),party=conn?.joinParty||1,at=now.toISOString();
   const R2={...r,joined:r.joined.filter((_,i)=>i!==idx),joinedNames:(r.joinedNames||[]).filter((_,i)=>i!==idx),partySize:Math.max(1,partyOf(r)-party),full:false};await store.set(`request/${rid}`,R2);
   if(conn)await store.set(`connection/${conn.id}`,{...conn,status:"removed",removedBy:userId,removedAt:at});
   const back=(await store.list("request/")).map(x=>x.value).find(x=>x?.userId===uid&&x.mergedInto===rid&&x.closedReason==="merged");if(back)await store.set(`request/${back.id}`,{...back,active:true,closedAt:null,closedReason:null,mergedInto:null,reopenedAt:at});
   const notes=[{to:uid,response:btn(`עדכון למשחק ${whenOf(r)}: הקבוצה עדכנה שאתם כבר לא חלק מהמשחק.${back?" הבקשה שלכם חזרה ללוח ואמשיך לחפש לכם התאמות.":""}`,[MY_REQ,MENU])}].concat(members(R2).filter(x=>x.id!==userId).map(x=>({to:x.id,response:btn(`עדכון למשחק ${whenOf(r)}: ${name} כבר לא בקבוצה. התפנה מקום, ואציע את המשחק לשחקנים מתאימים.`,[MENU])})));
   return{...btn(`הסרתי את ${name}. יש עכשיו ${partyOf(R2)} מתוך 4, והמשחק פתוח שוב להתאמות.`,[MY_REQ,MENU]),notifications:notes};}
