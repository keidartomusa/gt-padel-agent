...
return{text:`*הבקשות שלי* (${rows.length})\n\n${rows.map((r,i)=>`${i+1}. ${reqTitle(r)}\n${reqDesc(r)}`).join("\n\n")}\n\nלעריכה או מחיקה לחצו על 'לבקשות' ובחרו בקשה.`,list:{button:"לבקשות",sections:[{title:"בקשות פעילות",rows:rows.map(r=>({id:`req:${r.id}`,title:reqTitle(r).slice(0,24),description:reqDesc(r).slice(0,72)})).concat(rows.length<MAX_ACTIVE_REQUESTS?[{id:"players",title:"➕ בקשה חדשה"}]:[],LEAVE,{id:"menu",title:"לתפריט"})}]}};}
 ...
 if(input.startsWith("connect:")){const request=await store.get(`request/${input.slice(8)}`);if(!request||!request.active)return{text:"הבקשה כבר לא פעילה."};if(request.userId===userId)return{text:"זו הבקשה שלכם."};
   ...
   const members=[request.userId,...(request.joined||[])],isGroup=members.length>1,mp=mine?partyOf(mine):1,together=partyOf(request)+mp,who=mp===1?`${displayName} רוצה`:`${displayName} ועוד ${mp===2?"אחד":mp-1} רוצים`,
     // Tom 24.9 10:29 ("משדרג"): say how many are coming and the total ("תום ועוד 2 רוצים להצטרף לבקשה שלכם: ... יחד תהיו 4.").
     ask=btn(isGroup?`${who} להצטרף אליכם (${groupLabel(request)}): ${whenOf(request)} · רמה ${request.level}. יחד תהיו ${together}. לחבר? מספיק שאחד מכם יאשר.`:`${mp===1?`${displayName} רוצה להתחבר לבקשה שלכם`:`${who} להצטרף לבקשה שלכם`}: ${whenOf(request)} · רמה ${request.level}. יחד תהיו ${together}. לחבר ביניכם?`,[{id:`accept:${connectionId}`,title:"כן, לחבר"},{id:`decline:${connectionId}`,title:"לא מתאים"}]);
