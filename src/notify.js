...
export const MATCH_TEMPLATE={name:"gt_match_found",language:"he",payloads:["pending_yes","pending_no"]};
...
export async function deliver(store,to,response,{now=new Date(),send,sendTemplate}){
 ...
 if(windowOpen(profile,now))return{mode:"freeform",...await send(to,response)};
 ...
 if(!recent){result=await sendTemplate(to,MATCH_TEMPLATE);if(result.sent)pending.templateSentAt=now.toISOString();}
