...
// Same semantics as hebrew-when-corpus.test.js: expected start null = any; 4th field "ambiguous" = asking AM/PM is right.
export const caseOk=(c,date,start,asked)=>c[3]==="ambiguous"?(asked||(date===c[1]&&start===c[2])):(date===c[1]&&(c[2]==null||start===c[2]));
...
export async function evalProvider(name,adapter,corpus,{today,concurrency=6,timeoutMs=15000,fetchImpl=fetch}={}){
 ...
 async function worker(){while(i<corpus.length){const c=corpus[i++],[text,date,start]=c;const ctl=new AbortController(),t=setTimeout(()=>ctl.abort(),timeoutMs),t0=Date.now();
  try{const p=await adapter(text,today,{fetchImpl,signal:ctl.signal,onUsage:u=>{usage.input+=u.input;usage.output+=u.output;model=u.model||model;}});rows.push({text,ms:Date.now()-t0,date:p.date,start:p.startMinute,clar:p.needs_clarification,dateOk:p.date===date,startOk:c[3]==="ambiguous"?(p.needs_clarification==="ampm"||p.startMinute===start):(start==null||p.startMinute===start)});}
...
export function evalLocal(corpus,now){const rows=corpus.map(c=>{const p=parseIntentLocal(c[0],now);return caseOk(c,p.date,p.startMinute,!!p.ambiguousHour);});return{provider:"local",n:corpus.length,bothCorrect:rows.filter(Boolean).length};}
...
export async function evalCombo(gpt,jev,corpus,{today,concurrency=6,timeoutMs=15000,thresholds=[0,0.5,0.7,0.9]}={}){
 ...
 async function worker(){while(i<corpus.length){const c=corpus[i++],[text,date,start]=c,t0=Date.now();const [g,j]=await Promise.all([call("openai",gpt,text),call("jev",jev,text)]);
  const agree=!g.err&&!j.err&&g.p.date!=null&&g.p.date===j.p.date&&g.p.startMinute===j.p.startMinute;
  rows.push({text,ms:Date.now()-t0,agree,correct:agree&&caseOk(c,g.p.date,g.p.startMinute,false),conf:j.p?.confidence??0,g:g.p&&[g.p.date,g.p.startMinute],j:j.p&&[j.p.date,j.p.startMinute],want:[date,start]});}}
 await Promise.all(Array.from({length:concurrency},worker));
...
export async function evalHybrid(parse,corpus,now,{concurrency=6}={}){const rows=[];let i=0;
 async function worker(){while(i<corpus.length){const c=corpus[i++],[text,date,start]=c,t0=Date.now();const p=await parse(text,now,{log:false});const asked=!!(p.needsClarification||p.ambiguousHour);rows.push({text,ms:Date.now()-t0,source:p.source,asked,correct:caseOk(c,p.date,p.startMinute,asked)&&(!asked||c[3]==="ambiguous"),got:[p.date,p.startMinute],want:[date,start]});}}
 ...
 return{provider:"hybrid",n:rows.length,answered:rows.filter(r=>!r.asked).length,answeredCorrect:rows.filter(r=>!r.asked&&r.correct).length,wrongAnswers:rows.filter(r=>!r.asked&&!r.correct).length,askedCorrectly:rows.filter(r=>r.asked&&r.correct).length,askedUser:rows.filter(r=>r.asked).length,sources:src,latencyMs:{p50:pct(ms,0.5),p95:pct(ms,0.95),max:Math.max(...ms)},
