...
export async function resetStore(store,now=new Date(),commit=null){const keys=await store.keys("");let deleted=0,keptOptOuts=0,keptClicks=0;
 for(const k of keys){if(k.startsWith("booking-click/")){keptClicks++;continue;}if(k.startsWith("profile/")){const p=await store.get(k);if(p?.optedOutAt){await store.set(k,{userId:p.userId||k.slice(8),optedOutAt:p.optedOutAt});keptOptOuts++;continue;}}await store.delete(k);deleted++;}
