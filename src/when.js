...
export function localConfident(local,text){if(!local.dateExplicit)return false;const MON="ינואר|פברואר|מרץ|אפריל|מאי|יוני|יולי|אוגוסט|ספטמבר|אוקטובר|נובמבר|דצמבר",t=String(text).replace(/\d{1,2}[/.]\d{1,2}(?:[/.]\d{2,4})?/g," ").replace(new RegExp(`\\d{1,2}\\s*(?:ל|ב)?-?\\s*(?:${MON}|חודש)`,"g")," ").replace(/(?:ב)?עוד\s+\d{1,2}\s*(?:ימים|שבועות)/g," "),timeCue=/\d|בוקר|ערב|צהר|לילה|אחה|שעה|אחרי|לפני|מוצ|am\b|pm\b/i.test(t)||/(?:^|[\s\-])(?:[בלמ]|ב-)?(?:אחת|שתיים|שתים|שלוש|ארבע|חמש|שש|שבע|שמונה|תשע|עשר)(?=$|[\s,.?!])/.test(t);return !timeCue||local.startMinute!=null;}
