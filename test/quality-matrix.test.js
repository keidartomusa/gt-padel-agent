...
test("availability responses put exact Matchpointer deep links only in CTA metadata",async()=>{const {answerResponse}=await import("../src/agent.js"),r=await answerResponse("מגרש ב25/9 בשעה 17:00 ל90 דקות",{now:new Date("2026-09-23T08:00:00+03:00")});assert.doesNotMatch(r.text,/https?:|בדיקת זמינות בלבד/);const trackingUrl=new URL(r.messages[0].ctaUrl.url),target=new URL(trackingUrl.searchParams.get("target"));assert.equal(target.origin,"https://matchpointer.app");assert.equal(target.pathname,"/he/clubs/gt-padel/book");for(const key of ["court","time","date","duration","step"])assert(target.searchParams.get(key));assert.equal(target.searchParams.get("step"),"confirm");});
