...
test("availability replies include exact Matchpointer deep links from returned live slots",()=>{const text=formatHebrew({kind:"availability",date:"2026-09-25",slots:[{courtId:"0529104c-184e-44ef-b13b-0cecd3e758e1",courtName:"3",start:"17:00",end:"18:30",durationMinutes:90,price:225}]});assert.match(text,/https:\/\/matchpointer\.app\/he\/clubs\/gt-padel\/book\?/);assert.match(text,/court=0529104c-184e-44ef-b13b-0cecd3e758e1/);assert.match(text,/time=17%3A00/);assert.match(text,/date=2026-09-25/);assert.match(text,/duration=90/);assert.match(text,/step=confirm/);assert.match(text,/זו בדיקת זמינות בלבד, לא הזמנה/);});
