...
async function registration(store, n, { recurring, level, when, duration, party, court, flex, expectMatch }) {
...
for (let pair = 0; pair < 50; pair++) {
  const store = memoryStore(), n = pair * 2 + 1, recurring = pair % 2 === 1, level = levels[pair % 6], when = recurring ? "ימי חמישי אחרי 19:00" : "מחר אחרי 19:00", duration = [60, 90, 120][pair % 3], party = [1, 2, 3][pair % 3], court = pair % 4 !== 0, flex = [0, 30, 60][pair % 3];
  await registration(store, n, { recurring, level, when, duration, party, court, flex, expectMatch: false });
  const matching = pair % 3 !== 0;
  await registration(store, n + 1, { recurring, level: matching ? level : levels[(pair + 3) % 6], when: matching ? when : (recurring ? "ימי ראשון אחרי 08:00" : "מחר אחרי 08:00"), duration, party, court, flex, expectMatch: matching });
}
