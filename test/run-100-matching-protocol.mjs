...
async function registration(store, n, { recurring, level, when, duration, party, court, flex, expectMatch, expectNote = null }) {
  ...
  for (const s of [{ text: "שלום" }, { actionId: "players" }, { actionId: recurring ? "recurring" : "oneoff" }, { actionId: `level:${level}` }, { text: when }, { actionId: duration === "flex" ? "duration:flex" : `duration:${duration}` }, { actionId: `party:${party}` }, { actionId: `court:${court ? "yes" : "no"}` }, ...(court ? [] : [{ actionId: `flex:${flex}` }])]) turns.push(...await turnAll(user, s));
