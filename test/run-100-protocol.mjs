...
const TERMINAL = [
  ...
  ["left", t => /^מחקתי את כל הבקשות שלכם/.test(t.response.text || "")],
  ["stayed", t => /לא מחקתי כלום/.test(t.response.text || "")],
...
for (let j = 0; j < 20; j++) { const n = 71 + j, recurring = j % 4 === 3, u = makeUser(`97250300${String(n).padStart(4, "0")}`, `שחקן ${n}`); await convo(`${n}. רישום ${recurring ? "זמינות קבועה" : "משחק נקודתי"} ${levels[j % 6]}`, "matching", u, recurring ? [{ actionId: "players" }, { actionId: "recurring" }, { actionId: `level:${levels[j % 6]}` }, { actionId: `party:${[1, 2, 3][(j + 1) % 3]}` }, { text: "ימי שני ורביעי אחרי 20:00" }, { actionId: `duration:${[60, 90, 120][j % 3]}` }] : [{ actionId: "players" }, { actionId: recurring ? "recurring" : "oneoff" }, { actionId: `level:${levels[j % 6]}` }, { text: recurring ? "ימי שני ורביעי אחרי 20:00" : `מחר אחרי ${17 + (j % 5)}:00` }, { actionId: j % 7 === 0 ? "duration:flex" : `duration:${[60, 90, 120][j % 3]}` }, { actionId: `party:${[1, 2, 3][(j + 1) % 3]}` }, { actionId: `court:${j % 5 === 0 ? "no" : "yes"}` }, ...(j % 5 === 0 ? [{ actionId: `flex:${[0, 30, 60][(j + 2) % 3]}` }] : [])]); }
// 91-100: public board, brokered connection, settings
const reg = (u, lvl = "3–3.5", when = "מחר אחרי 19:00") => [{ actionId: "players" }, { actionId: "oneoff" }, { actionId: `level:${lvl}` }, { text: when }, { actionId: "duration:90" }, { actionId: "party:1" }, { actionId: "court:yes" }];
