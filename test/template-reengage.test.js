...
function graph({existing=[],manage=true}={}){const calls=[];let tpl=[...existing];return{calls,fetch:async(url,opt={})=>{calls.push({url,method:opt.method||"GET",body:opt.body});
 ...
 if(url.includes("/message_templates")&&opt.method==="POST"){tpl=[{name:"gt_match_found",status:"PENDING",category:"UTILITY",language:"he"}];return res({id:"T1",status:"PENDING",category:"UTILITY"});}
...
test("template definition: body matches the code, UTILITY, two quick replies in payload order, regular hyphens only",()=>{const d=templateDefinition();
 assert.equal(d.name,"gt_match_found");assert.equal(d.category,"UTILITY");assert.equal(d.language,"he");assert.equal(d.components[0].text,MATCH_TEMPLATE.body);
...
test("ensureTemplate does not re-submit an existing template; check-only never posts",async()=>{const g=graph({existing:[{name:"gt_match_found",status:"APPROVED",category:"UTILITY",language:"he"}]});
...
test("reengage: waits until 24h after the last message, then sends the fixed template exactly once",async()=>{sends.length=0;const s=await setup("2026-09-24T07:52:49Z");
 ...
 r=await reengageTest(s,{suffix:"0131",now:new Date("2026-09-25T08:37:00Z"),sendTemplate:okSend});assert.equal(r.status,"sent");assert.deepEqual(sends,[{to:U,t:"gt_match_found"}]);
